...
import os
import time
os.system("pip install -U youtube_dl && pip install -U youtube_search && pip install -U youtube-search-python && pip install -U youtube-search")
...
@BaseClient.on(X_cmd(pattern="yt(a|v)"))
...
@BaseClient.on(X_cmd(pattern="ytlink"))
async def hmm(ytwala):
    query = ytwala.pattern_match.group(2)
    if not query:
