...
@BaseClient.on(events.NewMessage(outgoing=True))  # pylint:disable=E0602
...
async def _(event):
    ...
    reason = re.search('pattern_match_group(1)', "1") or event.pattern_match.group(1) or bot.send_message("AFK Err")
...
@BaseClient.on(events.NewMessage(outgoing=True))
...
@BaseClient.on(X_cmd(pattern="night"))
async def _(event):
    if event.fwd_from:
        return
    global USER_night
